...
patches-own [
  grass ; grass length
  occupied? ; boolean value that determines whether that patch is occupied country
]
...
to setup-patches
  ask patches [
    ; setup occupied patches using the occupation percentage slider
    set occupied? (random-float 1.0 < occupation-percentage)
    ifelse occupied? [
      set pcolor red
      set grass 0
    ] [
...
to move  ;; turtle procedure
  rt random 360
  ; check if patch ahead is occupied, if it is
  ; make a turn, if it is not go to the patch
  ifelse [occupied?] of patch-ahead stride-length = true [
     rt random 360]
  [ fd stride-length ]
...
; Copyright 2024 Uri Wilensky, Farid Rasoolzadeh, Jan Baljan.
...
"cooperative" 1.0 0 -2064490 true "" "plot count cooperative-cows"
